...
// generic types for akxsystem layer2 blockchain
...
func (h *Hash) Scan(src interface{}) error {
	...
	hh, decodeErr := utils.DecodeHex(string(stringVal))
	if decodeErr != nil {
...
func (a Address) checksumEncode() string {
	...
	lowercaseHex := utils.EncodeToHex(addrBytes)[2:]
	hashedAddress := utils.EncodeToHex(utils.Keccak256(nil, []byte(lowercaseHex)))[2:]
...
func (a *Address) Scan(src interface{}) error {
	...
	aa, decodeErr := utils.DecodeHex(string(stringVal))
	if decodeErr != nil {
		return fmt.Errorf("unable to decode value, %w", decodeErr)
	}
